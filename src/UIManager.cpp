...
bool CPaintManagerUI::GetResourceFile( LPCTSTR pstrFileName, CStdString &strFileContent )
{
	...
	if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
		...
		if( hFile == INVALID_HANDLE_VALUE ) { TRACE(_T("Error opening file")); return false;}
		DWORD dwSize = ::GetFileSize(hFile, NULL);
		if( dwSize == 0 ) { TRACE(_T("File is empty")); return false;}
		if ( dwSize > 4096*1024 ) { TRACE(_T("File too large")); return false;}
		...
		if( dwRead != dwSize ) {
			delete[] pByte;
			TRACE(_T("Could not read file"));
			return false;
		}
		strFileContent.Assign((LPCTSTR)pByte, dwSize);
		delete[] pByte;
		...
		if( hz == NULL ) { TRACE(_T("Error opening zip file")); return false;}
		ZIPENTRY ze; 
		int i; 
		if( FindZipItem(hz, pstrFileName, true, &i, &ze) != 0 ) { TRACE(_T("Could not find ziped file")); return false;}
